...
const Agent = ({userName,userId,type,interviewId,questions}:AgentProps) => {
    ...
    const handleCall=async ()=>{
        ...
        if(type === 'generate'){
            ...
            if(questions){
                formateedQuestions=questions.map((question)=> `- ${question}`).join("/n");
            }
            ...
            <div className="w-full flex justify-center">
                {callStatus !== 'ACTIVE' ?(
                    <button className="relative btn-call" onClick={handleCall}>
                        <span className={cn('absolute animate-ring rounded-b-full opacity-75',callStatus !== 'CONNECTING' && 'hidden')}/>
                        <span>
